...
/* Local */
class PatternsMagazineSettingsApi {
    constructor() {
        this.endpoint = 'patterns-magazine/v1/settings';
    ...
    bindEvents() {
        document.addEventListener('click', (event) => {
            if (event.target.classList.contains('patterns-magazine-gsn-close')) {
                this.modifySettings('hide_get_started_notice', true);
            }
            else if (event.target.classList.contains('patterns-magazine-rn-permanent-close')) {
                this.modifySettings('remove_review_notice_permanently', true);
            }
            else if (event.target.classList.contains('patterns-magazine-rn-temporary-close')) {
                this.modifySettings('remove_review_notice_temporary_date_time', 1);
            }
            else if (event.target.classList.contains('patterns-magazine-install-plugins')) {
    ...
    updateUI() {
        if (this.settings.hide_get_started_notice) {
            const noticeElement = document.getElementById('patterns-magazine-gsn');
            if (noticeElement) {
        ...
        if (!this.isLoadReview()) {
            const reviewNoticeElement = document.getElementById('patterns-magazine-review-notice');
    ...
    async installPlugins() {
        const plugins = PatternsMagazineLocalize.recommended_plugins;
        ...
        window.location.replace(PatternsMagazineLocalize.theme_info_url);
    }
}

document.addEventListener('DOMContentLoaded', () => {
    new PatternsMagazineSettingsApi();
});
