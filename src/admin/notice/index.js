...
class PatternsMagazineSettingsApi {
	...
	bindEvents() {
		document.addEventListener( 'click', ( event ) => {
			...
			if ( target.closest( '.patterns-magazine-gsn-close' ) ) {
				this.modifySettings( 'hide_get_started_notice', true );
			} else if (
				target.closest( '.patterns-magazine-rn-permanent-close' )
			) {
				this.modifySettings( 'remove_review_notice_permanently', true );
			} else if (
				target.closest( '.patterns-magazine-rn-temporary-close' )
			) {
				this.modifySettings(
				...
				const installBtn = target.closest(
					'.patterns-magazine-install-plugins'
				);
				if ( installBtn ) {
					installBtn.classList.add( 'patterns-magazine-processing' );
					this.installPlugins();
				}
			}
		} );
	...
	updateUI() {
		...
		if ( ! this.isLoadReview() ) {
			const reviewNoticeElement = document.getElementById(
				'patterns-magazine-review-notice'
			);
